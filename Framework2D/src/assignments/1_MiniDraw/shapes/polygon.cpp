...
void Polygon::initial_point(){
    current_point_ = std::make_shared<Controlpoint>(0, 0);
    point_list_.push_back(current_point_);

    for(int i = 0; i < x_list_.size(); i++){
        current_point_ = std::make_shared<Controlpoint>(0, 0, Controlpoint::pointType::vertax, i);
        point_list_.push_back(current_point_);
    }
...
void Polygon::update_point(){
    ...
    for(int i = 0; i < x_list_.size(); i++){
        updating = point_list_[i + 1];
        updating->update(x_list_[i], y_list_[i]);
    }
}

void Polygon::update_shape_by_center(float x, float y){
...
void Polygon::update_shape_by_vertax(float x, float y){
...
float Polygon::min_distance(float x, float y){
